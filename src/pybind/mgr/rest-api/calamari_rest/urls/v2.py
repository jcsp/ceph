...
urlpatterns = patterns(
    ...
    # OSDs, Pools, CRUSH
    url(r'^cluster/crush_rule_set$', calamari_rest.views.v2.CrushRuleSetViewSet.as_view({'get': 'list'}),
        name='cluster-crush_rule_set-list'),
    url(r'^cluster/crush_rule$', calamari_rest.views.v2.CrushRuleViewSet.as_view({'get': 'list'}),
        name='cluster-crush_rule-list'),
    url(r'^cluster/pool$', calamari_rest.views.v2.PoolViewSet.as_view({'get': 'list', 'post': 'create'}),
        name='cluster-pool-list'),
    url(r'^cluster/pool/(?P<pool_id>\d+)$', calamari_rest.views.v2.PoolViewSet.as_view({
        'get': 'retrieve',
        'patch': 'update',
        'delete': 'destroy'}),
        name='cluster-pool-detail'),

    url(r'^cluster/osd$', calamari_rest.views.v2.OsdViewSet.as_view({'get': 'list'}), name='cluster-osd-list'),
    url(r'^cluster/osd/(?P<osd_id>\d+)$', calamari_rest.views.v2.OsdViewSet.as_view(
        {'get': 'retrieve', 'patch': 'update'}),
        name='cluster-osd-detail'),
    url(r'^cluster/osd /command$', calamari_rest.views.v2.OsdViewSet.as_view({'get': 'get_implemented_commands'})),
    url(r'^cluster/osd/(?P<osd_id>\d+)/command$', calamari_rest.views.v2.OsdViewSet.as_view(
        {'get': 'get_valid_commands'})),

    url(r'^cluster/osd/(?P<osd_id>\d+)/command/(?P<command>[a-zA-Z_]+)$', calamari_rest.views.v2.OsdViewSet.as_view(
        {'get': 'validate_command', 'post': 'apply'})),
    url(r'^cluster/osd_config$', calamari_rest.views.v2.OsdConfigViewSet.as_view({'get': 'osd_config', 'patch': 'update'})),

    url(r'^cluster/mon$', calamari_rest.views.v2.MonViewSet.as_view({'get': 'list'}),
        name='cluster-mon-list'),
    url(r'^cluster/mon/(?P<mon_id>[a-zA-Z0-9-\.]+)$', calamari_rest.views.v2.MonViewSet.as_view(
        {'get': 'retrieve'}), name='cluster-mon-detail'),
    url(r'^cluster/mon/(?P<mon_id>[a-zA-Z0-9-\.]+)/status$', calamari_rest.views.v2.MonViewSet.as_view(
    ...
    url(r'^cluster/server$', calamari_rest.views.v2.ServerClusterViewSet.as_view({'get': 'list'}),
    ...
    url(r'^cluster/config$', calamari_rest.views.v2.ConfigViewSet.as_view({'get': 'list'})),
