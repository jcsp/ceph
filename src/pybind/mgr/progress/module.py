        ...
        for pg in self._pgs:
            ...
            if "active" in states and "clean" in states and not unmoved:
                ...
                if info['stat_sum']['num_bytes'] == 0:
                    ...
                    complete_accumulate += float(info['stat_sum']['num_bytes_recovered'] - self._original_bytes_recovered[pg]) / (info['stat_sum']['num_bytes'])
...
class Module(MgrModule):
    ...
    def _osd_out(self, old_map, old_dump, new_map, osd_id):
        ...
        # TODO: keep a record of when OSDs were marked down, so that we can
        # add some retrospective info to the event telling the timeline
        # story of how it was marked down, then out, then completed.
        ev = PgRecoveryEvent(
