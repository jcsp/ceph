...
static PyObject *osdmap_pg_to_up_acting_osds(PyObject *self, PyObject *args)
{
  PyObject *mapobj;
  int pool_id = 0;
  int ps = 0;
  if (!PyArg_ParseTuple(args, "Oii:pg_to_up_acting_osds",
			&mapobj, &pool_id, &ps)) {
    return nullptr;
  }
  ...
  osdmap->pg_to_up_acting_osds(pg_id,
      &up, &up_primary,
